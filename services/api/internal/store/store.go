...
func (s *Store) ListMembers(ctx context.Context, householdID int64) ([]domain.Member, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&member.ID, &member.HouseholdID, &member.Name, &member.CreatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) ListExpenses(ctx context.Context, householdID int64) ([]domain.Expense, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT id, household_id, member_id, category_id, amount_cents, currency, note, spent_at, created_at
		FROM expenses
		WHERE household_id = ?
...
func (s *Store) CreateExpense(ctx context.Context, householdID int64, input domain.CreateExpenseInput) (domain.Expense, error) {
	...
	result, err := s.db.ExecContext(ctx, `
		INSERT INTO expenses (household_id, member_id, category_id, amount_cents, currency, note, spent_at, created_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?)
	`, householdID, input.MemberID, input.CategoryID, input.AmountCents, input.Currency, input.Note, input.SpentAt.UTC(), now)
	if err != nil {
